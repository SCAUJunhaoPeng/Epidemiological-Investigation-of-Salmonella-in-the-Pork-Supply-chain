...
auto_pal <- function(nms) { pal <- hue_pal(l = 45, c = 60)(length(nms)); names(pal) <- nms; pal }
...
circos.clear()
circos.par(gap.after = rep(6, length(nodes)), track.margin = c(0.01, 0.01))
...
asym <- pair_tab %>%
  rowwise() %>%
  mutate(total = AB + BA, p = binom_p(AB, BA)) %>%
  ungroup() %>%
...
# Bar plot
stars <- function(p) if (p < 0.001) "***" else if (p < 0.01) "**" else if (p < 0.05) "*" else "ns"
